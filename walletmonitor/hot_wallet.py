...
import pytz
import requests
from database import DatabaseManager, Wallet
    ...
    def update_top_wallets(
        ...
        with self.db.get_connection() as conn:
            ...
            for wallet in wallets:
                if wallet["address"] not in uniq_wallets and wallet[
                    "address"
                ].startswith("0x"):
                    wallet = Wallet(
                        address=wallet["address"],
                        friendly_name=wallet["name"],
                        grp_name=base,
                        grp_type="Hot2",
                    )
                    uniq_wallets[wallet.address] = wallet
                    self.db.get_or_create_wallet(conn, wallet)
            conn.commit()
...
if __name__ == "__main__":
    ...
    new_hots = """binance,kraken,coinbase,okx,bybit,opensea,gnosis,circle,gate.io,aave,wintermute,bitfinex,crypto.com,cumberland,bitget,kucoin,copper,bitvavo,bitstamp,mexc,chainflip,deribit,uniswap,bitso,falconx,htx,bullish.com,singleownermsca,independent,bitgo,gsr-markets,base,bitflyer,upbit,walletsimple,gemini,forwarder,bitmart,phemex,proxy,bitkub,abraxas-capital-heka-fund"""
    bases = [
        "bybit",
        "binance",
        "okx",
